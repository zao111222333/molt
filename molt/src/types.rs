...
pub type CommandFunc<Ctx> = fn(&mut Interp<Ctx>, &[Value]) -> MoltResult;
...
pub struct Subcommand<Ctx: 'static> {
  pub name: &'static str,
  pub func: Box<dyn Fn(&mut Interp<Ctx>, &[Value]) -> MoltResult>,
}

impl<Ctx: 'static> Subcommand<Ctx> {
  pub fn new(
    name: &'static str,
  ...
  pub fn find<'a>(
    ensemble: &'a [Subcommand<Ctx>],
    sub_name: &str,
  ) -> Result<&'a Subcommand<Ctx>, Exception> {
    for subcmd in ensemble {
      if subcmd.name == sub_name {
        return Ok(subcmd);
      }
    }

    let mut names = String::new();
    names.push_str(ensemble[0].name);
    ...
    if ensemble.len() > 2 {
      let vec: Vec<&str> = ensemble[1..last].iter().map(|x| x.name).collect();
      names.push_str(&vec.join(", "));
    }

    if ensemble.len() > 1 {
      names.push_str(", or ");
      names.push_str(ensemble[last].name);
    }

    molt_err!("unknown or ambiguous subcommand \"{}\": must be {}", sub_name, &names)
  }
}
